const detailData = () => {
    ...
    const renderAnimeDetails = (array, itemId) => {
        ...
        if (animeObj) {
            ...
            console.error('Аниме с itemId=' + itemId + ' не найдено!');
            preloader.classList.remove('active')
        }
    }
    ...
    fetch('./db.json')
        ...
        .then((data) => {
            ...
            console.log('Полученные данные:', data.anime.length, 'аниме');
            console.log('Запрошенный itemId:', ganreParams);

            if (ganreParams) {
                renderAnimeDetails(data.anime, ganreParams)
            } else {
                console.log('itemId не указан в URL');
                preloader.classList.remove('active')
            }
